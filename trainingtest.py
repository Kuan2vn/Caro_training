            ...
            while not done_action2:
              ...
              for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        action3, prob, val = agent2.choose_action(observation2)

                        action, reward2, done, done_action2 = env.take_action_human(*event.pos)
                        
                        pygame.display.update()
                        temp += 1
                        if temp <= 30:
                            # Nghiên cứu sửa
                            # thiết kế xem agent2 học theo cách nào, cần nhớ cái gì

                            agent2.remember(observation2, action, prob, val, reward2, done)
